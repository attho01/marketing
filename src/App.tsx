...
성공적인 마케팅 전략 수립을 위해, 먼저 귀사 **[제품/브랜드의 핵심 특징이나 스펙]**을 선택해 주시거나 아래 입력창을 통해 직접 설명해 주세요.`,
  ...
  // Helper: Call Gemini API directly from browser (useful for client-only deployments like Vercel)
  const verifyClientSide = async (key: string): Promise<boolean> => {
  ...
  const callGeminiDirectly = async (apiKey: string, model: string, prompt: string, systemInstruction?: string): Promise<string> => {
    ...
    try {
      ...
      if (response.ok && contentType.includes("application/json")) {
        ...
        if (data.success) {
          ...
          setVerifyMessage("Gemini API Key가 서버를 통해 정상적으로 승인 및 활성화되었습니다! 이제 모든 마케팅 기획서 작성 단계에 해당 키가 우선 적용됩니다.");
      ...
      // If server returned non-JSON, we fall back to client-side direct verification
      console.log("Server verification returned non-JSON. Falling back to direct client-side verification...");
      const success = await verifyClientSide(trimmed);
      if (success) {
        setUserApiKey(trimmed);
        setVerifyStatus("success");
        setVerifyMessage("Gemini API Key가 브라우저 직접 통신(Client-side)을 통해 성공적으로 검증 및 활성화되었습니다! Vercel 등 프론트엔드 단독 배포 환경에서도 안전하게 사용이 가능합니다.");
      }
    } catch (err: any) {
      console.log("Error during server verification, trying client-side verification as fallback:", err);
      try {
        const success = await verifyClientSide(trimmed);
        if (success) {
          setUserApiKey(trimmed);
        ...
        let friendlyMsg = clientErr.message || "";
        if (friendlyMsg.includes("API_KEY_INVALID") || friendlyMsg.includes("API key not valid") || friendlyMsg.includes("INVALID_ARGUMENT") || friendlyMsg.includes("400")) {
          friendlyMsg = "입력하신 Gemini API Key가 유효하지 않거나 올바르지 않습니다. Google AI Studio에서 발급받은 'AIzaSy'로 시작하는 정확한 키 값을 입력해 주시기 바랍니다.";
        }
        setVerifyStatus("error");
        setVerifyMessage("API Key 승인에 실패했습니다: " + friendlyMsg);
      }
    } finally {
      setIsVerifying(false);
    }
...
`;
    ...
    try {
      ...
      console.log("Server suggestions unavailable. Falling back to client-side direct call...");
      if (!userApiKey) {
        throw new Error("No API key available for client-side fallback");
      }
      const textResult = await callGeminiDirectly(userApiKey, "gemini-3.5-flash", prompt);
      const parsedExamples = parseJsonArray(textResult);
      setDynamicSuggestions(parsedExamples);
    } catch (e) {
      console.error("Failed to load suggestions via server/client, using local fallback:", e);
      if (currentStep === 2) {
        setDynamicSuggestions([
...
3. 예산 구간별 채널 믹스 기준 엄격 반영
   사용자가 입력한 예산 규모("${bud}")를 정밀하게 평가하여, 아래의 4가지 구간 중 어디에 매핑되는지 결정하십시오. 구간 판단의 근거를 기획안 1절 도입부에 구체적인 한 문장으로 명시하고 이에 맞는 채널들만 사용해 실행안을 기획하십시오.
...
제시해 드린 기획안의 특정 컨셉의 톤앤매너, 슬로건, 마케팅 채널, 예산 배분 등을 보정하고 싶으시면 언제든 아래 입력창에 수정 요청 사항을 남겨주세요. 기존의 성공적인 기획 구조를 유지하면서 **해당하는 특정 부분만 즉시 디테일하게 재작성**해 드립니다.`,
      ...
      // Fallback to direct client-side call
      console.log("Server generation unavailable or returned non-JSON. Falling back to direct client-side call...");
      if (!userApiKey) {
        throw new Error("No API key available for client-side fallback");
      }
      const clientProposal = await callGeminiDirectly(userApiKey, "gemini-3.5-flash", prompt);
      ...
      setMessages((prev) => [
        ...prev.filter((m) => m.id !== loadingMsgId),
        {
          id: `proposal-complete-${Date.now()}`,
          sender: "assistant",
          text: `정조준한 타겟과 예산 규모 분석에 따른 전략적 캠페인 기획안 초안이 마침내 수립되었습니다! 우측 패널(또는 기획서 탭)에서 깔끔하게 정리된 완성본을 상시 열람하실 수 있습니다. (Client-side Fallback 엔진 사용)
...
제시해 드린 기획안의 특정 컨셉의 톤앤매너, 슬로건, 마케팅 채널, 예산 배분 등을 보정하고 싶으시면 언제든 아래 입력창에 수정 요청 사항을 남겨주세요. 기존의 성공적인 기획 구조를 유지하면서 **해당하는 특정 부분만 즉시 디테일하게 재작성**해 드립니다.`,
      ...
      setMessages((prev) => [
        ...prev.filter((m) => m.id !== loadingMsgId),
        {
          ...
          sender: "assistant",
          text: `기획안 생성 중 예상치 못한 오류가 발생했습니다. 아래 다시 시도하기 버튼을 눌러 다시 기획안을 도출하거나, 이전 단계를 다시 체크할 수 있습니다. (오류: ${e.message})`,
          timestamp: new Date(),
        },
      ]);
    } finally {
...
다른 컨셉안들의 일관성은 완벽히 유지하면서 요청하신 특정 파트만 베테랑 디렉터의 시각으로 날카롭게 다듬어 오겠습니다. 잠시만 대기해 주세요.`,
    ...
    const prompt = `
당신은 10년 차 이상의 베테랑 브랜드 마케팅 디렉터입니다. 기존에 작성된 캠페인 기획서와 사용자의 수정 요청 피드백을 깊이 있게 이해하고, 특정 부분만 예리하게 수정하여 업데이트된 최신 기획서를 한국어로 작성해 주세요.
...
당신은 베테랑 디렉터로서 아래의 개정 지침을 한치의 오차도 없이 100% 준수해야 합니다.

[작성 지침 및 제약조건]
1. 기존 기획서에서 수정 요청한 특정 내용만 똑똑하게 변경하십시오. 수정 요청이 없었던 다른 컨셉이나 기대효과, 일관성 있는 뼈대는 완전히 유지 및 보존해야 합니다.
2. 각 컨셉의 제목 바로 밑에 있는 \`(포지셔닝 축: OOO)\` 및 실행 버전 라벨 등의 마크다운 구조와 네이밍 형식을 철저히 준수하십시오.
3. 수정 후에도 글자 수 및 세부 제약사항을 만족시켜야 합니다:
   - 슬로건: 한국어 기준 20자 내외(±5자, 즉 공백 포함 15자~25자 사이)
   - 컨셉 정의: 2~3문장 이내
   - 버전별 세부 실행안: 각 버전당 정확히 2문장 이내 (소구 메시지 1문장 + 구체적 채널/아이디어 1문장)
4. '잘', '적절히', '창의적인', '위트 있게', '효과적으로' 등 모호하고 주관적인 수식어는 절대 사용하지 마십시오.
5. 본문 문장의 말줄임표(...) 사용 금지 및 마침표(.) 완결 구조 준수.
6. 기획서의 가장 마지막 라인에는 반드시 다음 문장과 일치하는 글을 한 줄 추가하십시오:
   "특정 컨셉의 톤앤매너, 채널, 예산 배분을 조정하고 싶으시면 말씀해 주세요. 해당 부분만 재작성해 드립니다."

[출력 포맷 (기존 기획서의 마크다운 형식을 그대로 보존하여 출력)]
`;
...
추가로 보완하고 싶으신 부분(톤앤매너, 채널 믹스 조정 등)이 있으시면 언제든지 편하게 다시 말씀해 주세요.`,
      ...
      // Fallback to direct client-side call
      console.log("Server revision unavailable or returned non-JSON. Falling back to direct client-side call...");
      if (!userApiKey) {
        throw new Error("No API key available for client-side fallback");
      }
      const clientRevisedProposal = await callGeminiDirectly(userApiKey, "gemini-3.5-flash", prompt);
      setProposal(clientRevisedProposal);
      ...
      setMessages((prev) => [
        ...prev.filter((m) => m.id !== loadingMsgId),
        {
          id: `proposal-revised-${Date.now()}`,
          sender: "assistant",
          text: `요청하신 피드백이 완벽하게 업데이트되었습니다! 기획서 대시보드에 수정이 완료된 최신 개정판을 반영해 두었습니다. (Client-side Fallback 엔진 사용)
